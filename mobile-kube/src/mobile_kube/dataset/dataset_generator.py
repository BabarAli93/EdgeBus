...
class DatasetGenerator:
    def __init__(self, nums, metrics, nodes_cap_rng,
        ...
        # updating this piece of code to check that each 'nodes_cap_rng' have both cpu and ram
        for _, key in nodes_cap_rng.items():
            assert len(key) == self.num_resources, "each node is not given cpu and ram"

        # following was the original statement which is updated by the above loop
        #assert len(nodes_cap_rng) == self.num_resources

        self.cutoff = cutoff
        assert len(self.cutoff) == self.num_resources, \
            ("number of elements in the cutoff",
             " is not equal to the number of resources")
        # TODO need to update for each node
        for num, key in nodes_cap_rng.items():
            self.nodes_rng_ram = key['ram']
            self.nodes_rng_cpu = key['cpu']
            self.nodes_rng_ram['min'] *= self.cutoff['ram']
            self.nodes_rng_ram['max'] *= self.cutoff['ram']
            self.nodes_rng_cpu['min'] *= self.cutoff['cpu']
            self.nodes_rng_cpu['max'] *= self.cutoff['cpu']

        self.nodes_capacities = nodes_cap_rng
    ...
    def _make_capacities(self):
        ...
        """   
        # 1. generate the nodes_resources
        #    doing it in a while statement to make sure
        ...
        for _ in range(tries_limit):
            ...
            #nodes_capacities_ = self.nodes_capacities
            nodes_ram = np.zeros((self.num_nodes, 1))
            for index, key in self.nodes_capacities.items():
                nodes_ram_range = np.arange(start=key['ram']['min'],
                                            stop=key['ram']['max']
                                                 + key['ram']['step'],
                                            step=key['ram']['step'])
                nodes_ram[int(index)] = nodes_ram_range
            ...
            nodes_cpu = np.zeros((self.num_nodes, 1))
            for index, key in self.nodes_capacities.items():
                nodes_cpu_range = np.arange(start=key['cpu']['min'],
                                            stop=key['cpu']['max']
                                                 + key['cpu']['step'],
                                            step=key['cpu']['step'])
                nodes_cpu[int(index)] = nodes_cpu_range
