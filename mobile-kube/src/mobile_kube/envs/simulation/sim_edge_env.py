...
class SimEdgeEnv(SimBaseEnv):
    def __init__(self, config: Dict[str, Any]):
        ...
        self.hetero_reward = 0
        self.num_overload_count = 0
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, int, bool, dict]:
        ...
        if self.no_action_on_overloaded and self.num_overloaded > 0:
            ...
            self.num_overload_count += self.num_overloaded
            print(f"Overload count: {self.num_overload_count}")
        ...
        self.users_stations = self.edge_simulator.sample_users_stations(
            timestep=self.timestep)      # this line will make user movement, update user stations and user distances
        ...
        reward, rewards = self._reward(
            ...
            hetero_reward=self.hetero_reward
            )
