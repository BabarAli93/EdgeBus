...
class KubeBaseEnv(gym.Env):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.penalty_overuse: float = config['penalty_overuse']
        ...
        #print(f"Printing Services Nodes: {self.services_nodes_obj}")
    ...
    @property
    def num_overusage(self) -> int:
        """it should return the number of nodes utilising more than 70% resources
        """
        overused_nodes = np.unique(np.where(
            self.nodes_resources_request_frac > 0.7)[0])
        return len(overused_nodes)
