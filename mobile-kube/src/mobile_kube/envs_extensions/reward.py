...
def _reward(
    ...
    reward_overuse = _reward_overused(self, num_overused)
    ...
    reward_total = reward_latency + reward_consolidation +\
        reward_move + reward_variance + reward_overuse + \
        hetero_reward
    rewards = {
        ...
        "reward_overusage": reward_overuse,
        "hetero_reward": hetero_reward
    }
...
def _reward_latency_5(self, users_distances: np.array) -> Tuple[float, Dict[str, Any]]:
    ...
    # normalise distances with the largest distance
    users_distances_sum = np.sum(users_distances)
    #print(users_distances_sum)
    reward_raw = 1/users_distances_sum
    users_distances_min_sum = self.average_station_node * self.num_users
    if users_distances_min_sum != 0:
        max_reward = 1/users_distances_min_sum
    else:
        max_reward = 0.115
...
# ------------- Overusage Rewards ---------------
def _reward_overused(self, num_overused: int):
    overuse_factor = num_overused / self.num_nodes
    reward_overuse = self.penalty_overuse * overuse_factor
    return reward_overuse
