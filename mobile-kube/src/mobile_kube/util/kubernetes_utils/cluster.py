...
class Action(BaseFunctionalities):
    ...
    def move_pod(
        ...
        if (previousService.spec.cluster_ip is None) and (previousService.spec.cluster_i_ps is not None):
            previousService.spec.cluster_i_ps = []

        #print(f"Previous Service Specs: {previousService.spec}")
...
class Cluster:
    ...
    def __init__(
        ...
        #current_context = config.kube_config.list_kube_config_contexts()[0]
        #print(current_context[0]['context']['user'])
