...
def config_dataset_generation_check(config: Dict[str, Any]):
    ...
    assert len(config['nodes_cap_rng']) == \
           config_nums['nodes'], (f"number of nodes <{config_nums['nodes']}> is "
                                  "not equal to the number of node capacities in "
                                  f"nodes_cap_rng: <{len(config['nodes_cap_rng'])}> ")
...
def env_config_base_check(config: Dict[str, Any]):
    ...
    """
    # check the items
    allowed_items = ['obs_elements', 'penalty_illegal', 'penalty_overuse', 'penalty_move',
    ...
    for item in ints:
        assert type(config[item]) == int, f"<{item}> must be an integer"
    floats = ['penalty_illegal', 'penalty_illegal', 'penalty_overuse',
