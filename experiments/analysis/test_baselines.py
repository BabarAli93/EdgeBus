...
def run_experiments(
    *, test_series: int, train_series: int, type_env: str,
    dataset_id: int, workload_id: int, network_id: int, network_id_test: int,
    ...
    algorithm = {
        ...
        'kube-greedy': 'latency_greedy',
        'sim-mango': 'sim-mango',
        'kube-mango': 'kube-mango'}[type_env]
    ...
    for i in range(0,1):
        ...
        while not done:
            ...
            moves = info['num_moves']
            num_moves += moves
            states.append(state)
            episode_reward += reward
        print(f"Episode Num Moves : {num_moves}")
...
@click.option('--type-env', required=True,
              type=click.Choice(['sim-binpacking', 'sim-greedy', 'sim-mango',
                                 'kube-binpacking', 'kube-greedy', 'kube-mango']),
              default='sim-greedy')
...
@click.option('--network-id-test', required=False, type=int, default=1)
...
def main(test_series: int, comp_train_series: int, type_env: str,
    ...
    run_experiments(
        ...
        network_id=network_id, network_id_test=network_id_test,
        trace_id=trace_id, trace_id_test=trace_id_test,
