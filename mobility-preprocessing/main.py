...
def parallel_etl(first, last, interval, path, output_path):
    etl = ETL(path)
    etl.extract()
...
@click.command()
@click.option('--dataset', '-d', default="data/*", help="Directory of Cabspotting data set", show_default=True,
              type=str)
...
@click.option('--interval', '-i', help="Enter the intervals between two points in seconds", default=100, type=int,
              show_default=True)
@click.option('--processes', '-p', help="number of processes", default=2, type=int, show_default=True)
@click.option('--output', '-o', help="output directory to save the results", default="cleaned_dataset")
def main(dataset, get, url, interval, processes, output):
    ...
    files = [os.path.join(os.getcwd(), '') + file for file in glob.glob(dataset)]
